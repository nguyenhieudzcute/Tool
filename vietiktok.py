...
class Zefoy:
	...
	def __init__(self):
		...
		self.url = 'None'
		self.text = 'Nguyên blue'
		url1=input("[Nguyên blue]</> Nhập link video: ")
		self.url=url1
	...
	def get_captcha(self):
		...
		try:
			...
			print('[Nguyên blue]</> Đang giải capcha..')
			return False
		except Exception as e:
			print(f"[Nguyên blue]</> Không thể giải captcha: {e}")
			time.sleep(2)
			self.get_captcha()

	def send_captcha(self, new_session = False):
		if new_session: self.session = requests.Session(); os.remove('session'); time.sleep(2)
		if self.get_captcha(): print('[Nguyên blue]</> Đang kêt nối đến session');return (True, 'The session already exists')
		...
		if 'Enter Video URL' in request.text: 
			print('[Nguyên blue]</> Session đã được tạo')
			open('session','w',encoding='utf-8').write(self.session.cookies.get('PHPSESSID'))
			print(f"[Nguyên blue]</> Giải capcha thành công: {captcha_solve}")
	...
	def find_video(self):
		...
		while True:
			...
			if 'Session expired. Please re-login' in self.video_info: print('[Nguyên blue]</> Phiên hết hạn. Đang đăng nhập lại...');self.send_captcha(); return
			elif 'service is currently not working' in self.video_info: return (True,'[Nguyên blue]</> Dịch vụ hiện không hoạt động, hãy thử lại sau. | Bạn có thể thay đổi nó trong cấu hình.')
				...
				elif lamtilo >= 1000: print('[Nguyên blue]</> Your IP was banned')
				# print(f'[Nguyên blue]</> Thời gian chờ để tiếp tục tăng: {t} giây')
				# for lam in range(t,0,-1):
				# 	lam-=1
				# 	print(f"[Nguyên blue]</> Vui lòng chờ: {lam}",end="\r")
				# 	time.sleep(1)
				_=time.time()
				while time.time()-2<_+lamtilo:
					t-=1
					print("[Nguyên blue]</> Vui lòng chờ: {0} ".format(t)+"giây .", end="\r")
					time.sleep(1)
	...
	def use_service(self):
		...
		if 'Session expired. Please re-login' in res: print('[Nguyên blue]</> Phiên hết hạn. Đang đăng nhập lại...');self.send_captcha(); return ""
		elif 'Too many requests. Please slow' in res: time.sleep(3)
		elif 'service is currently not working' in res: return ('[Nguyên blue]</> Dịch vụ hiện không hoạt động, hãy thử lại sau. | Bạn có thể thay đổi nó trong cấu hình.')
	...
	def get_video_id(self, url_ = None, set_url=True):
		...
		if request.text == '': print('[Nguyên blue]</> Link video không hợ lệ'); return False
	...
	def update_name(self):
		while True:
			try:
				...
				self.text = f"[Nguyên blue]</> | Views: {video_info['viewCount']} "
			except: pass
			time.sleep(5)
trang = "\033[1;37m"
xanh_la = "\033[1;32m"
...
╱╱╱╱╱╭━╯┃╱╱╭━╯┃
╱╱╱╱╱╰━━╯╱╱╰━━╯
    {xanh_la} </> thắc mắc gì thì liên hệ admin nguyên blue nha! </>
    \033[1;37m────────────────────────────────────────────────────────────
    {do}[{trang}{kt_code}{do}] {vang}TOOL BUFF VIEW TIKTOK 
    {do}[{trang}{kt_code}{do}] \033[1;35mADMIN:\033[1;36m Nguyên blue
    {do}[{trang}{kt_code}{do}] \033[1;31mmessenger: \033[1;33m m.me/nguyenblue2007
    {do}[{trang}{kt_code}{do}] {xanh_la}FACEBOOK:\033[1;37m facebook.com/nguyenblue2007
    \033[1;37m────────────────────────────────────────────────────────────\n"""
...
#     print(f"{trang}[Nguyên blue]</> Link key: {link1}")
     
# else:
#     link1=""
# nhap = input(f"{trang}[Nguyên blue]</> Nhập key vừa lấy được: ")
# if nhap == key:
if 1:
    print(f"{xanh_la}[Nguyên blue]</> Key đúng mời bạn vô tool")
    logo()
    Z = Zefoy()
    ...
    print(f"{do}[Nguyên blue]</> Key sai vui lòng vượt lại")
    exit()

while True:
	try: 
		if 'Service is currently not working, try again later' in str(Z.use_service()): print(f'{do}[Nguyên blue]</>Dịch vụ hiện không hoạt động, hãy thử lại sau. | Bạn có thể thay đổi nó trong cấu hình.');time.sleep(5)
